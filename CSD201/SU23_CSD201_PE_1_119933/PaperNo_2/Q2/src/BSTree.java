...
public class BSTree {
    ...
    */
    void insert(String xOwner, int xPrice, int xColor) {
        ...
        Node q = root;
        while(q!= null) {
            if(q.getInfo().price == xPrice) {
                return;
            }
            if(xPrice > q.getInfo().price) {
                f =q;
                q = q.right;
            }
            else {
        ...
        else if(p.getInfo().price > f.getInfo().price) {
            f.right = p;
        }
        else{
            f.left = p;
    ...
    void addNode() {
        int k = count(root);
        insert("V", 100 - k + 3, 2*k -6);
    }
    int count(Node p) {
        return(p == null) ? 0 : 1 + count(p.right) + count(p.left);
    }
